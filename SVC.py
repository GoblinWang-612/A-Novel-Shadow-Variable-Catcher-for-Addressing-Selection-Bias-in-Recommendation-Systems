...
def user_shadow_train_eval(config):
    ...
    if config["show_log"]:
        ...
        plt.title("iVAE")
        plt.legend()
        plt.savefig(data_params["Shadow_path"] + "SVC_loss.png") # 存檔不顯示
        plt.close()
...
if __name__ == '__main__':
    ...
    if args.tune:
        ...
        if args.data_params["name"] == 'coat':
            ...
            u_layer_dim_search = [1024] 
            u_shadow_dim_search = [16]  
        ...
        search_grid = {'lr': [1e-3, 5e-4, 1e-4], 
                       'weight_decay': [1e-4, 1e-5, 1e-6], 
                       'u_layer_dim': u_layer_dim_search, 
                       'u_shadow_dim': u_shadow_dim_search}
