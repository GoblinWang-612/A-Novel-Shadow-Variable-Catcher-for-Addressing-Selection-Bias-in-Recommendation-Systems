...
def s0_density_train_eval(config):
    ...
    optimizer = torch.optim.AdamW(params=real_nvp_s0.parameters(), lr=config["lr"],
    ...
    for epoch in range(config["epochs"]):
        ...
        for index, Shadow_s0 in enumerate(S0_train_dataloader):
            optimizer.zero_grad()                        
            loss = real_nvp_s0.nll(Shadow_s0) 
            loss.backward()
            optimizer.step()
    ...
    if config['showfigandlog']:
        ...
        plt.legend()
        plt.savefig(data_params["Shadow_path"] + "S0_loss.png") 
        plt.close()
...
def s1_density_train_eval(config):
    ...
    optimizer = torch.optim.AdamW(params=real_nvp_s1.parameters(), lr=config["lr"],
                                 weight_decay=config["weight_decay"])
    ...
    for epoch in range(epochs):
        ...
        for index, Shadow_s1 in enumerate(S1_train_dataloader):
            ...
            loss = real_nvp_s1.nll(Shadow_s1) 
    ...
    if config['showfigandlog']:
        ...
        plt.legend()
        plt.savefig(data_params["Shadow_path"] + "S1_loss.png") 
        plt.close()
...
if __name__ == '__main__':
    ...
    if args.tune:
        print("--tune model--")
        search_grid = {'lr': [1e-3, 5e-4, 1e-4, 5e-5, 1e-5], 
        ...
        print("Best Val:", best_val, "Best Params:", best_param)
    else:
        Train_eval_config = {
            "seed": 1208,
            "epochs": 1000,
