...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dims, activations):
        ...
        for i, (hidden_dim, activation) in enumerate(zip(hidden_dims, activations)):
            ...
            #將 relu/lrelu 替換為 GELU
            if activation == "relu" or activation == "lrelu" or activation == "gelu":
                self.linear_nets.add_module("act_{}".format(i), nn.GELU())
...
class RealNVP_MLP(nn.Module):
    def __init__(self, input_size, layer_num, layer_dim, output_size):
        ...
        for _ in range(layer_num):
            layers.append(ResidualBlock(layer_dim))
...
class AffineTransform(nn.Module):
    def __init__(self, type, input_size=2, layer_num=2, layer_dim=64, device="cpu"):
        ...
        self.mlp = RealNVP_MLP(input_size=input_size, layer_num=layer_num, layer_dim=layer_dim, output_size=input_size * 2).to(device)
